...
function cartHandler(opt={}) {
	...
	for(let i=0; i<cartGroups.length; i++) {
		...
		let number = cartGroups.eq(i).find('.op-cont input').val();
		let cart = new Cart(cartId, single, number);
		cartList.push(cart);
	}
	...
	function selectHandler() {
		...
		selectAllChks.on('click', function(e) {
			if(this.checked){
				...
				//取消全选
				selectAllChks.prop('checked', false);
	...
	function getCheckedCartItem() {
		...
		for(let i=0; i<chkboxs.length; i++) {
			...
			*/
			if(chkboxs.eq(i).prop('checked')) {
				// isNull = false;
				++ chkNum;
				goodIds += ('&goodId=' + cartList[i].getCartId);
				//console.log(goodIds);
		...
		if(!chkNum) {
			...
			payBtn.attr('href', topayUrl + goodIds);
