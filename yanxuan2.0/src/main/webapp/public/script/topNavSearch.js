...
function search(searchObj, ajaxUrl, sendUrl, objs = {}) {
	this.$elem = searchObj;
		...
		timer = setTimeout(() => {
			...
			$.ajax({
				...
				success(data) {
					...
					//loadDefaultKeyword(data);
					loadSuggestKeyword(data);
				}
			});
		}, 300);
	...
	//通过ajax加载默认推荐关键词
	this.loadDefaultKeyword = function(data) {
		if(data) return;
	...
	//通过ajax加载推荐关键词面板
	this.loadSuggestKeyword = function(data) {
