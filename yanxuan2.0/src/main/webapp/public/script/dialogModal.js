...
function loadModal(opt={}) {
	...
	let {loginUrl, addrUrl} = Object.assign({}, defaultOpt, opt);
	...
	tiggerBtn.on('click', function() {
		...
		switch(type) {
			case 'login':// 登录模态框
				...
				if(isload) {
					modalWrap.load(loginUrl + ' .yx-login-wrap', function() {
			...
			case 'address': //添加地址模态框
				...
				if(isload) {
					let loadUrl = addrUrl + ' .address-modal'
					modalWrap.load(loadUrl, function() {
						//pages/components/modal/address.html
						let closeBtn = modalWrap.find('.close');
						closeModal(closeBtn);
